...
This project is for the EEG workshop in Haifa.

## Project Standards
- Language: Python (prefer marimo notebooks for analysis). Version: >=3.9,<3.12
- Libraries (from `pyproject.toml`): numpy, scipy, pandas, matplotlib, mne, mne-bids, autoreject, pyvista, dash, flask, plotly, jupyter, jupyterlab, ipykernel, marimo, tqdm, joblib, pyyaml
- Documentation: Use Docstrings for functions and Markdown cells in notebooks.

## Context
- Focus on EEG signal processing, artifact removal, and feature extraction.
- Data is likely in `.edf`, `.bdf`, or `.fif` formats.

## Environment Setup (Windows 11 PowerShell)
Text-UI style script that installs dependencies, clones the repo, and boots into marimo or JupyterLab.
